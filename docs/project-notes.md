...
- **Поиск по блогу** — поле `#bq` на `/blog/`; `app.js` фильтрует `<li>` списка `#bloglist` по атрибуту `data-search` (заголовок+описание+слаг), пустой результат → блок `#bnores`. Без индекса — фильтрация уже отрендеренного DOM (статей мало).
